...
create table $SOURCE_SCHEMA.course_metadata as \
  ...
  from read_parquet('/users/output/$SOURCE_SCHEMA/course_metadata/course_metadata.parquet') as tb1; \
create table $SOURCE_SCHEMA.courseid_cpr_mapping as \
  select ROW_NUMBER() OVER() AS _id, * \
  from read_csv_auto('/users/output/$SOURCE_SCHEMA/course_id_cpr_mapping.txt'); \
create table $SOURCE_SCHEMA.diagnoses_procedures as \
  ...
  from read_parquet('/users/output/$SOURCE_SCHEMA/diagnoses_procedures/diagnoses_procedures.parquet') as tb1 \
...
create table $SOURCE_SCHEMA.prescriptions as \
  ...
  from read_parquet('/users/output/$SOURCE_SCHEMA/drugs/prescriptions.parquet') as tb1; \
create table $SOURCE_SCHEMA. observations as \
  select ROW_NUMBER() OVER() AS id, tb1.* \
  replace(concat(tb1.variable, '-', tb2.description) as variable) \
  from read_parquet('/users/output/$SOURCE_SCHEMA/observations/observations-*.parquet') as tb1 \
  inner join (select * from read_parquet('/users/output/$SOURCE_SCHEMA/descriptions.parquet')) as tb2 \
  on tb1.from_file=tb2.filename and tb1.variable=tb2.field;"

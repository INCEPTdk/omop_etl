...
CREATE TEMP TABLE cteDrugTarget AS
SELECT
    d.DRUG_EXPOSURE_ID,
    d.PERSON_ID,
    c.CONCEPT_ID,
...
/* / */
CREATE TEMP TABLE cteEndDates AS
SELECT
    PERSON_ID,
                ...
                FROM
                    (
                        ...
                        FROM
                            cteDrugTarget
                        UNION
                        ALL -- add the end dates with NULL as the row number, padding the end dates by 30 to allow a grace period for overlapping ranges.
                        SELECT
                            PERSON_ID,
                            INGREDIENT_CONCEPT_ID,
...
/* / */
CREATE TEMP TABLE cteDrugExpEnds AS
SELECT
    d.PERSON_ID,
...
SELECT
    ...
    COALESCE(
        ...
        (CONDITION_START_DATE + 1 * INTERVAL '1 day')
                ...
                FROM
                    (
                        ...
                        FROM
                            cteConditionTarget
                        UNION
                        ALL -- pad the end dates by 30 to allow a grace period for overlapping ranges.
                        SELECT
                            PERSON_ID,
                            CONDITION_CONCEPT_ID,
